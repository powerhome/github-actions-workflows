...
A kit used in four files or fewer is listed exhaustively and the plan says so. A kit used more widely lists a sample of up to eight call sites alongside its true total, and the plan says the coverage is representative — the tier is derived in the formatter from the use count rather than taken from the provider, so a sample cannot be published as exhaustive. Changed paths belonging to no kit — tokens, global props, the `pb_rails` helper layer — are collected separately, because they apply across the application rather than to one page.

### The Playbook plan

When the delta finds changed kits, the run switches to the profile's `playbook_prompt` and renders through `TestPlan::Playbook::Parser` and `TestPlan::Playbook::Formatter` instead of the standard pair. The profile resolves from the label before any lockfile has been read, so the choice is made after the delta and named as a step output, and the render step follows the same decision the provider was given.

That plan is a different document. It leads with a coverage table — kit, uses in the application, whether the coverage is complete or representative, and the case range — then the cases themselves, grouped by kit. There is no separate regression section: a Playbook raise adds nothing, so every case in it is a regression test and the plan says so up front. There is no permissions section either, since a version bump changes no permissions; the role needed to reach a page rides on the case instead. A closing section carries what belongs to no kit: cross-cutting Playbook changes, and any other dependency the pull request raised, each traced to a changed path or a raise rather than offered as general advice.
